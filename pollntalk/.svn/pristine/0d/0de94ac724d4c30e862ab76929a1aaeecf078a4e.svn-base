...
try 
{
    ...
    $count      = $board->getboardcount("1");
    $page       = $_GET["page"];
    $paging     = $board->makePaging($count, $page);
    $result     = $board->getBoardList("1", $paging);
} 
catch (CException $ex) 
{
    $ex->printException();
...
?>
					...
					<div class="boardListButtonBox">
						<div class="buttonLeftBox">
							<a id="boardprev" href="/admin_manager.php?mode=boardnoticelist&page=<?php echo($paging["boardprev"])?>">
...
?>
    						<div class="buttonLeftBox">
								<a id="page" href="/admin_manager.php?mode=boardnoticelist&page=<?php echo($i)?>">
