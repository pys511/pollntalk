...
var questionUpPostions		= new Map();
var questionDownPostions	= new Map();
var questionFileUploads		= new Map();
var answerFileUploads		= new Map();
var questionDeletes			= new Map();
var appendAnswers			= new Map();
var removeAnswers			= new Map();
...
//업로드된 파일 받기
var setVoteFile	= function(fileData, question_index, answer_index)
{
...
function initQuestion()
{
	...
	//up 이벤트 일괄 제거
	for (let [key, value] of questionUpPostions) 
		key.removeEventListener("click", value, false);
	questionUpPostions.clear();
	
	//down 이벤트 일괄 제거
	for (let [key, value] of questionDownPostions) 
		key.removeEventListener("click", value, false);
	questionDownPostions.clear();
	...
	//응답 추가 이벤트 일괄 제거
	for (let [key, value] of appendAnswers)
		key.removeEventListener("click", value, false);
	appendAnswers.clear();
	
	//응답 삭제 이벤트 일괄 제거
	for (let [key, value] of removeAnswers)
		key.removeEventListener("click", value, false);
	removeAnswers.clear();
		
	//질문 이미지/동영상 선택 버튼 이벤트 일괄 제거
	for (let [key, value] of questionResourceTypes)
		key.removeEventListener("click", value, false);
	questionResourceTypes.clear();
	
	//응답 이미지/동영상 선택 버튼 이벤트 일괄 제거
	for (let [key, value] of answerResourceTypes)
		key.removeEventListener("click", value, false);
	answerResourceTypes.clear();
		
	//질문 이미지/동영상 처리 버튼 이벤트 일괄 제거
	for (let [key, value] of questionResourceViews)
		key.removeEventListener("click", value, false);
	questionResourceViews.clear();
	
	//응답 이미지/동영상 처리 버튼 이벤트 일괄 제거
	for (let [key, value] of answerResourceViews)
		key.removeEventListener("click", value, false);
	answerResourceViews.clear();
	...
	for (var i = 0; i < questionItems.length; i++)
	{
		...
		//위로 처리
		var upPos		= newQuestionList.querySelector(".upPos");
		var upPosFunc 	= questionUpPostion.bind(null, question_index.value);
		upPos.addEventListener("click", upPosFunc, false);
		questionUpPostions.set(upPos, upPosFunc);
		
		//아래로 처리
		var downPos		= newQuestionList.querySelector(".downPos");
		var downPosFunc = questionDownPostion.bind(null, question_index.value);
		downPos.addEventListener("click", downPosFunc, false);
		questionDownPostions.set(downPos, downPosFunc);
...
function initAnswer(answer_lst, questionIndex)
{
	...
	for (var index = 0; index < answerItems.length; index++)
	{
		...
		//응답 추가
		var addAnswer			= newAnswerItem.querySelector(".appendAnswer");
		addAnswer.setAttribute("id", "addAnswer_" + questionIndex + "_" + index);
		var appendAnswerFunc	= appendAnswer.bind(null, questionIndex)
		addAnswer.addEventListener("click", appendAnswerFunc, false);
		appendAnswers.set(addAnswer, appendAnswerFunc);
		
		//응답 삭제
		var delAnswer			= newAnswerItem.querySelector(".removeAnswer");
		delAnswer.setAttribute("id", "removeAnswer_" + questionIndex + "_" + index);
		var removeAnswerFunc	= removeAnswer.bind(null, questionIndex, index);
		delAnswer.addEventListener("click", removeAnswerFunc, false);
		removeAnswers.set(delAnswer, removeAnswerFunc);
		...
		var answerCorrectLabel	= newAnswerItem.querySelector(".answer_correct_label");
		answerCorrectLabel.setAttribute("for", "answer_correct_" + questionIndex + "_" + index);
...
function appendQuestion()
{
	...
	newQuestionList.setAttribute("class", "votewriteitem newWriteItem");
...
window.addEventListener("load", function()
{
	...
	for (var i = 0; i < resource_type.length; i++)
	{
		...
		var doResourceTypeFunc	= doResourceType.bind(null, resourctItem.id, vote_resource_url.id, resource_view.id, voteImageFile.id, voteVideoFile.id, voteResourceType.id, null);
		resourctItem.addEventListener("click", doResourceTypeFunc);
	}
	
	var resource_view		= document.getElementById("resource_view");
	var resourceViewFunc	= doResourceView.bind(null, "vote_real_name", "vote_temp_path", "vote_resource_type", "voteImageFile", "voteVideoFile", "vote_resource_url");
	...
	registerVote.addEventListener("click", function()
	{
		...
		var isValue		= false;
		for (var i = 0; i < voteKind.length; i++)
		{
			if (voteKind[i].checked)
			{
				isValue	= true;
				break;
			}
		...
		{
			if (answer_kinds[i].options[answer_kinds[i].selectedIndex].value == "0")
			{
		...
		for (var i = 0; i < question_indexs.length; i++)
		{
			...
			var answer_lst		= document.getElementById("answer_lst_" + questionIndex);
			if (answer_lst.style.display == "none")
				continue;
	...
	{
		var frmVote		= document.getElementById("frmVote");
		frmVote.action	= "/admin_controller.php?mode=vote_form_delete";
		frmVote.submit();
	});
