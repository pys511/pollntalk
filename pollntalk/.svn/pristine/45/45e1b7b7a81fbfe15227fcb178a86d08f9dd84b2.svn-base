...
var questionUpPostion = function(index)
{
	...
	while((previousSibling = currentElement.previousSibling) != null)
	{
...
function initAnswer(answer_lst, questionIndex)
{
	...
	for (var index = 0; index < answerItems.length; index++)
	{
		...
		//윈도우 응답 추가
		var winPlues		= newAnswerItem.querySelectorAll(".appendAnswer");
		for (var i = 0; i < winPlues.length; i++)
		{
			...
			appendAnswers.set(addAnswerItem, appendAnswerFunc);
		}
		
		//윈도우 응답 삭제
		var winMinuses		= newAnswerItem.querySelectorAll(".removeAnswer");
		for (var i = 0; i < winMinuses.length; i++)
		{
			...
			removeAnswers.set(delAnswerItem, removeAnswerFunc);
		}
