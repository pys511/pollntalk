...
function insertEditFile(frameId, formId)
{
	...
	frmFileUpload.action	= "/admin_controller.php?mode=mail_file";
...
function insertEditImage(frameId, formId)
{
	...
	var frmFileUpload		= document.getElementById(formId);
	frmFileUpload.target	= frameId;
	frmFileUpload.action	= "/admin_controller.php?mode=mail_image";
...
function setEditerImage(fileData)
{
	...
	var strFilePath		= fileData.temp_path;
	//주소확인 필요!!!!!
	var strFilePath     = "http://pollntalk2021.cafe24.com/" + fileData.temp_path;
...
function deleteMail(formID)
{
	var mailNum		= document.getElementById("num");
	if (mailNum.value == "")
	{
	...
	var frmMail		= document.getElementById(formID);
...
function registerMail(formId, toId, subjectId, editorId, contextId, viewfilelistId, inputfilelistId, inputimagelistId)
{
	...
	var mailTo = document.getElementById(toId);
	if (mailTo.value == "")
	{
		alert("수신인을 입력하셔야 합니다.");
		mailTo.focus();
		return;
	}
	
	var mailSubject	= document.getElementById(subjectId);
...
window.addEventListener("load", function()
{
	...
	allMember.addEventListener('change', (event) => {
  	if (event.target.checked) 
	{
    	document.getElementById("mailTo").value = "ALL";
  	} 
	else 
	{
